...
def parse_args():
    ...
    parser.add_argument("--positive-threshold", type=float, default=0.8, help="Similarity threshold for positive pairs.")
    parser.add_argument("--hard-negative-min", type=float, default=0.4, help="Minimum similarity for hard negatives.")
    ...
    parser.add_argument("--min-similarity", type=float, default=0.2, help="Minimum similarity retained from fuzzy matcher.")
...
def main():
    ...
    result = pipeline.run()
    dataset = result.get("resolution", {}).get("dataset")
    if dataset is None or dataset.empty:
        raise SystemExit("No dataset rows generated; check thresholds or inputs.")
